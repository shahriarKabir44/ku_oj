...
export default function SubmissionsContainer({
  ...
  const [codeSubmissionModalVisibility, setCodeSubmissionModalVisibility] =
    React.useState(false);
  React.useEffect(() => {}, [currentUser, problem]);
  ...
  return (
    <div className="submissionsContainer card">
      <h3
        style={{
          margin: 0,
        }}
          ...
          onClick={() => {
            if (!currentUser) {
              ModalManager.showAlert(
                "you must log in first to submit solutions",
                  ...
                  <tr key={index}>
                    <td>
                      <Link
                        ...
                        to={`${Global.CLIENT_URL}/viewSubmission/${problem.contestId}/${submission.id}`}
                      >
      ...
      <SubmitCode
        ...
        open={codeSubmissionModalVisibility}
        handleClose={() => {
